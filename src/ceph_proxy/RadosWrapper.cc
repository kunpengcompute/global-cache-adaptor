...
static void RadosBindCore(std::vector<uint32_t> coreId, uint64_t ii)
{
    cpu_set_t set;
    CPU_ZERO(&set);

    for (auto cpuId : coreId) {
        CPU_SET(cpuId, &set);
    }
    pid_t tid = ii;
...
static void FindRadosThread(pid_t pid, std::vector<uint64_t> &vecBindMsgr, std::vector<uint64_t> &vecBindDispatch,
    std::vector<uint64_t> &vecBindFnRadosClient)
{
    ...
    if (r != 0) {
        ProxyDbgLogErr("readir (%s) failed: %d", path, r);
        return;
    }
    ...
    for (auto &i : ls) {
        ...
        if (!feof(fp)) {
            ...
            if (strLine.find("fn-radosclient") != string::npos) {
                vecBindFnRadosClient.push_back(atoll(i.c_str()));
            }
        }
        fclose(fp);
    }
...
{
    std::vector<uint64_t> vecBindMsgr;
    std::vector<uint64_t> vecBindDispatch;
    std::vector<uint64_t> vecBindFnRadosClient;
    std::vector<uint32_t> bindCore;

    FindRadosThread(pid, vecBindMsgr, vecBindDispatch, vecBindFnRadosClient);

    if (coreId.size() > vecBindMsgr.size() + vecBindDispatch.size()) {
        std::copy(coreId.begin(), coreId.begin() + vecBindMsgr.size() + vecBindDispatch.size(),
            std::back_inserter(bindCore));
    } else {
        bindCore = coreId;
    }

    for (auto &ii : vecBindMsgr) {
        ProxyDbgLogInfo("RadosBindMsgrWorker bind msgr %llu", ii);
        RadosBindCore(bindCore, ii);
    }

    for (auto &ii : vecBindDispatch) {
        ProxyDbgLogInfo("RadosBindMsgrWorker bind dispatcher %llu", ii);
        RadosBindCore(bindCore, ii);
    }

    for (auto &ii : vecBindFnRadosClient) {
        ProxyDbgLogInfo("RadosBindMsgrWorker bind fn-radosclient %llu", ii);
        RadosBindCore(bindCore, ii);
    }

    ProxyDbgLogInfo("totalCoreNum=%d, msgrNum=%d, dispatchNum=%d fs-radosclient=%d, bind %d core(s)",
        coreId.size(), vecBindMsgr.size(), vecBindDispatch.size(), vecBindFnRadosClient.size(), bindCore.size());
}
