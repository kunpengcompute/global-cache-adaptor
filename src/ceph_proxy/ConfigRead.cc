...
#define DEFAULT_CEPH_KEYRING_PATH  "/etc/ceph/ceph.client.admin.keyring"
...
typedef struct {
    ...
    uint64_t osdTimeout;
    uint64_t monTimeout;
    uint32_t ioWorkerNum;
    uint32_t radosCtxNum;
} ProxyControlCfg;
...
static void InitClusterCfg()
{
    ...
    sprintf(g_ProxyCfg.cephKeyringPath, "%s", DEFAULT_CEPH_KEYRING_PATH);
    ...
    g_ProxyCfg.osdTimeout = 0;          // 0表示不设置超时时间
    g_ProxyCfg.monTimeout = 0;          // 0表示不设置超时时间

    g_ProxyCfg.ioWorkerNum = 1;
    g_ProxyCfg.radosCtxNum = 1;
}
...
static int32_t ReadConfig()
{
    ...
    ret = GetCfgItemUint32(&g_ProxyCfg.ioWorkerNum, "proxy", "proxy_worker_queue_num");
    if (ret != RETURN_OK) {
        ProxyDbgLogErr("Get Config Item [unit:proxy,key:proxy_worker_queue_num] failed");
        return RETURN_ERROR;
    }
    ret = GetCfgItemUint32(&g_ProxyCfg.radosCtxNum, "proxy", "proxy_rados_ctx_num");
    if (ret != RETURN_OK) {
        ProxyDbgLogErr("Get Config Item [unit:proxy,key:proxy_rados_ctx_num] failed");
        return RETURN_ERROR;
    }
    return ret;
}
...
uint32_t ProxyGetIoWorkerNum()
{
    return g_ProxyCfg.ioWorkerNum;
}
uint32_t ProxyRadosCtxNum()
{
    return g_ProxyCfg.radosCtxNum;
}