...
// conf parser method, only used in ReadConfig930
extern "C" {
...
static int32_t SetCfgCorenum()
{
    ...
    if (cpuNum < CPU_NUM_96) {
        ...
        ret2 = GetCfgItemCstr(
            g_SaClusterControlCfg.workerCore, sizeof(g_SaClusterControlCfg.workerCore), "sa", "sa_worker_core_64");
        ...
        ret2 = GetCfgItemCstr(
            g_SaClusterControlCfg.workerCore, sizeof(g_SaClusterControlCfg.workerCore), "sa", "sa_worker_core_96");
        ...
        ret2 = GetCfgItemCstr(
            g_SaClusterControlCfg.workerCore, sizeof(g_SaClusterControlCfg.workerCore), "sa", "sa_worker_core_128");
        keyCpuNum = CPU_NUM_128;
    } else {
        ret = GetCfgItemCstr(
            g_SaClusterControlCfg.coreNumber, sizeof(g_SaClusterControlCfg.coreNumber), "sa", "core_number_256");
        ret2 = GetCfgItemCstr(
            g_SaClusterControlCfg.workerCore, sizeof(g_SaClusterControlCfg.workerCore), "sa", "sa_worker_core_256");
    ...
    if (ret2 != RETURN_OK) {
        fprintf(stderr, "config get config para [unit:sa, key:sa_worker_core_%d] failed.\n", keyCpuNum);
        return RETURN_ERROR;
    }
    return RETURN_OK;
}
...
#define GET_CFG_ITEM_U64(dest, unit, key)                                                     \
    ...
    uint64_t readStatFlag;
    GET_CFG_ITEM_U64(&readStatFlag, "cache", "rcache_snapshot_enable");
    if (readStatFlag == 0) {
        g_SaClusterControlCfg.readStat = false;
    } else {
        g_SaClusterControlCfg.readStat = true;
    }
...
std::string OsaConfigRead::GetWorkerCore()
{
    return g_SaClusterControlCfg.workerCore;
}
...
bool OsaConfigRead::GetReadStatFlag()
{
    return g_SaClusterControlCfg.readStat;
}
